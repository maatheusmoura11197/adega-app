...
if menu == "📦 Gestão de Estoque":
    ...
    with aba_cad:
        ...
        if modo_cadastro == "Selecionar Existente":
            ...
            # Placeholder adicionado (sem value)
        ...
        if st.button("💾 Salvar Estoque", type="primary"):
            if nome_final and qtd_total_adicionada > 0:
                with st.spinner("Calculando e Salvando..."):
                    ...
                    if not encontrado:
                        # Colunas: Nome|Tipo|Forn|Custo|Venda|Estoque|Data|QtdFardo
    ...
    with aba_ver:
        if not df_estoque.empty:
            ...
            # Formatação Monetária e Cálculo de Lucro
            ...
            cols_show = ['Nome', 'Visual Estoque', 'Preço Venda', 'Custo Médio', 'Lucro Real', 'Data']
...
# ==========================================
    ...
    # 2. CARRINHO (VISUALIZAÇÃO SINCRONIZADA E IMUTÁVEL)
    st.markdown("### 🛒 O que ele está levando?")
    
    if not df_estoque.empty:
        # Cria lista com ESTOQUE VISUAL NO NOME
        # Ex: SKOL LATA (Estoque: 50)
        ...
        # Extrai o nome real (remove a parte do estoque visual)
        if prod_escolhido_menu != "(Apenas Pontuar - Sem Produto)":
            # Pega tudo antes do " (Estoque:"
        ...
        if not erro:
            with st.spinner("Processando..."):
                ...
                if nome_produto_real != "(Apenas Pontuar - Sem Produto)" and total_unidades_venda > 0:
                    ...
                    if idx_est != -1:
                        if est_atual >= total_unidades_venda:
                            ...
                            # Log com Data e Valor Monetário
                            valor_total_venda = total_unidades_venda * venda_val
                            sheet_hist_est.append_row([pegar_data_hora(), nome_produto_real, "VENDA", total_unidades_venda, f"R$ {valor_total_venda:.2f}", f"Cli: {nome_input}"])
                        else:
                ...
                if cliente_ja_existe:
                    ...
                    if is_new_client: # Atualiza nome se for correção
